...
const getLatestRelease = () => {
  return new Promise((resolve, reject) => {
    const opts = {
      host: 'api.github.com',
      path: '/repos/esimov/triangle-app/releases',
      headers: {
        'User-Agent': `TriangleApp v${currentVersion}`
      }
    }
    https.get(opts, res => {
      let json = ''
...
module.exports = () => {
  getLatestRelease().then(release => {
    ...
    if (version && semver.gt(version, currentVersion)) {
      ...
      dialog.showMessageBox({
        title: TITLE,
        message: `You are using latest version of ▲ TRIANGLE (${currentVersion})`,
        buttons: []
      })
    }
  }).catch(err => {
